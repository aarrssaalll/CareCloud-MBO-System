  ...
  async createUsers(departments, teams) {
    ...
    for (const user of users) {
      const result = await this.pool.request()
        ...
        .query(`
          INSERT INTO mbo_users (employeeId, email, firstName, lastName, name, role, title, phone, hireDate, status, departmentId, teamId, createdAt, updatedAt)
          OUTPUT INSERTED.*
          VALUES (@employeeId, @email, @firstName, @lastName, @name, @role, @title, @phone, GETDATE(), @status, @departmentId, @teamId, GETDATE(), GETDATE())
        `);
